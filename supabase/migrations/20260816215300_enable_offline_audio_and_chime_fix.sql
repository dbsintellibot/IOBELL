-- Migration: Enable offline audio caching and fix double chime playing
-- Description: Updates public.get_device_config(device_mac text) to:
--   1. Return correct audio_id matching audio_files.id for non-precombined MP3 schedules.
--   2. Add 'pc' (precombined) boolean flag to schedule items to signal the ESP32 to bypass local chime.
