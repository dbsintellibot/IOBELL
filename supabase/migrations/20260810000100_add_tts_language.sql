-- Migration: Add tts_language to bell_times and default_tts_language to schools
-- Description: Adds language support (English, Urdu, Arabic) for Text-to-Speech config.

-- 1. Modify schools table
ALTER TABLE public.schools 
ADD COLUMN IF NOT EXISTS default_tts_language text DEFAULT 'en';

-- Add check constraint for default_tts_language
ALTER TABLE public.schools
DROP CONSTRAINT IF EXISTS check_default_tts_language;

ALTER TABLE public.schools
ADD CONSTRAINT check_default_tts_language CHECK (default_tts_language IN ('en', 'ur', 'ar'));

-- Update existing records to default
UPDATE public.schools 
SET default_tts_language = 'en'
WHERE default_tts_language IS NULL;

-- 2. Modify bell_times table
ALTER TABLE public.bell_times 
ADD COLUMN IF NOT EXISTS tts_language text DEFAULT 'en';

-- Add check constraint for tts_language
ALTER TABLE public.bell_times
DROP CONSTRAINT IF EXISTS check_tts_language;

ALTER TABLE public.bell_times
ADD CONSTRAINT check_tts_language CHECK (tts_language IN ('en', 'ur', 'ar'));

-- Update existing records to default
UPDATE public.bell_times
SET tts_language = 'en'
WHERE tts_language IS NULL;


-- 3. Re-create restore_school_data RPC supporting default_tts_gender, default_tts_language, and tts_language
...
BEGIN
    ...
    -- 2. Restore School Settings (including default_tts_gender and default_tts_language)
    UPDATE public.schools
    SET
        ...
        default_tts_gender = COALESCE(p_school_row->>'default_tts_gender', default_tts_gender),
        default_tts_language = COALESCE(p_school_row->>'default_tts_language', default_tts_language),
        updated_at = now()
    ...
    -- 6. Restore Bell Times (supporting tts_gender, tts_language and all scheduling columns)
        ...
        BEGIN
            IF v_mapped_profile_id IS NOT NULL THEN
                ...
                VALUES (
                    ...
                    COALESCE(r_bell_time.tts_language, 'en'),
                    now()
                );
