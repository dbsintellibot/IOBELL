-- Restore device overrides in public.get_device_config(device_mac)
-- Fix bug where empty audio_url is returned for precombined TTS schedules when pre-announcement is disabled.
        ...
        ELSE
            ...
            SELECT json_agg(sched) INTO v_schedule_data FROM (
                SELECT
                    ...
                    COALESCE(MAX(t.label), '') as lb,
                    CASE
                        -- FIXED: Return precombined path if chime enabled OR if chime disabled but precombined file doesn't have the chime anyway (format mismatch)
