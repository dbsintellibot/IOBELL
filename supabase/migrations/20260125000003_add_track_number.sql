-- Add track_number to audio_files and update get_device_config

-- 1. Add track_number column
ALTER TABLE public.audio_files ADD COLUMN IF NOT EXISTS track_number integer DEFAULT 1;

-- 2. Update get_device_config to include track_number
    ...
    BEGIN
        ...
        SELECT json_agg(t) INTO v_schedule_data FROM (
            SELECT 
                ...
                af.duration,
                COALESCE(af.track_number, 1) as track_number -- Return track number (default 1)
