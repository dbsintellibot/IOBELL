-- Migration: Mandatory Pre-Announcement System Fix (Zero Firmware Changes)
-- Description: Wraps scheduled MP3 and TTS audio in combine-audio Edge Function
-- while returning pa_en = false in top-level device config so ESP32 firmware
-- streams Chime -> 3s Silence -> Bell MP3/TTS as one continuous audio stream.
...
BEGIN
    ...
    BEGIN
        ...
        SELECT json_agg(sched) INTO v_schedule_data FROM (
            SELECT
                ...
                CASE
                    -- When pre-announcement is enabled and chime URL exists: combine chime + delay + schedule audio
                    WHEN v_pa_enabled AND v_pa_url IS NOT NULL AND v_pa_url != '' THEN
                        'https://hjlwzkwiweocnfztshmy.supabase.co/functions/v1/combine-audio?chime_url=' || public.urlencode(v_pa_url) || '&delay_sec=' || v_pa_delay || '&audio_url=' || public.urlencode(
                            CASE 
                    ...
                    -- For TTS audio without pre-announcement: direct Google TTS stream
                    WHEN t.play_type = 'tts' AND t.tts_message IS NOT NULL AND t.tts_message != '' THEN
                        'https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=en&q=' || public.urlencode(t.tts_message)
                    -- For MP3 audio without pre-announcement: clean storage path
                    ELSE
    ...
    -- Return pa_en = false in top-level JSON so ESP32 firmware does not play standalone double chimes.
    -- The chime + delay + bell/TTS audio is combined server-side into a single audio stream by combine-audio.
