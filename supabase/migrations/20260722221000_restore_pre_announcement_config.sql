-- Migration: Restore Pre-Announcement Config and Fix Audio URL Slashes
-- Description: Restores top-level pa_en and pa_url so live announcements, voice notes, and TTS broadcasts trigger pre-announcements,
-- while ensuring scheduled bell audio_urls preserve forward slashes '/' (folder/file.mp3) so Supabase Storage resolves files correctly.
        ...
        SELECT json_agg(sched) INTO v_schedule_data FROM (
            SELECT
                ...
                CASE
                    ...
                    -- For MP3 audio: return clean storage path preserving slashes (e.g. folder/period%201.mp3)
                    ELSE
