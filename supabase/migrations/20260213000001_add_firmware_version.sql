-- Add firmware_version column to bell_devices table
...
-- Create register_device_from_esp function for ESP32 firmware with firmware_version
...
BEGIN
    ...
    IF v_device_id IS NOT NULL THEN
        -- Device exists: Update status, name, and firmware version
        UPDATE bell_devices
        SET 
            status = 'online',
            last_heartbeat = now(),
    ...
    ELSE
        ...
        INSERT INTO bell_devices (mac_address, name, status, school_id, last_heartbeat, firmware_version)
        VALUES (p_mac_address, p_device_name, 'online', v_school_id, now(), p_firmware_version)
...
GRANT EXECUTE ON FUNCTION public.register_device_from_esp(text, text, text, text) TO anon, authenticated, service_role;
