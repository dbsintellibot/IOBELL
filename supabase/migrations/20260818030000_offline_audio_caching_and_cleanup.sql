-- Migration: Offline Audio Caching & Cleanup
-- 1. Clean up public.audio_files table by deleting all auto-generated combined audio entries
-- 2. Update get_device_config(device_mac text) to strictly filter Audio Manager files and return pa_url + all_audio_files

-- 1. Clean up auto-generated rows in audio_files table (they belong in bell_times.precombined_at)
DELETE FROM public.audio_files 
WHERE storage_path LIKE 'combined/%';

-- 2. Update get_device_config function
...
BEGIN
    ...
    -- Fetch strictly real Audio Manager files for the school (excluding combined schedule audio)
        ...
        ELSE
            ...
            SELECT json_agg(sched) INTO v_schedule_data FROM (
                SELECT
                    ...
                    COALESCE(MAX(t.label), '') as lb,
                    CASE
                        -- Return precombined path if chime enabled OR if chime disabled but precombined file doesn't have the chime anyway
