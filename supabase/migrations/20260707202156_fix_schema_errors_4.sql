-- Fix remaining schema mismatch errors: drop location, location_area, volume, board_type, input_power_type from restore_school_data
...
BEGIN
    ...
    -- Removed location, location_area, volume, board_type, input_power_type
    ...
    FOR r_bell_time IN SELECT * FROM jsonb_to_recordset(p_bell_times) AS x(profile_id uuid, bell_time time, day_of_week integer[], audio_file_id uuid, play_type text, tts_message text)
    LOOP
