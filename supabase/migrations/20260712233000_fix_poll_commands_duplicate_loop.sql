-- Migration: 20260712233000_fix_poll_commands_duplicate_loop.sql
-- Description: Update poll_commands to mark the returned command as 'executed' immediately, preventing duplicate execution loops.
...
BEGIN
    ...
    IF v_cmd.id IS NOT NULL THEN
        -- Update the command status to 'executed' immediately on the server
        -- to ensure it is only delivered once and cannot cause an infinite loop.
