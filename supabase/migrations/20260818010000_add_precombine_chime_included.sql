-- Migration: Add precombine_chime_included column and update get_device_config
-- Description: Adds a flag to track whether a precombined file actually includes the chime, ensuring the device plays the chime separately if mismatched.

-- 1. Reset precombined_at for all MP3 period bells to ensure they play cleanly
UPDATE public.bell_times
SET precombined_at = NULL
WHERE play_type = 'mp3' OR play_type IS NULL;

-- 2. Add precombine_chime_included column
ALTER TABLE public.bell_times 
ADD COLUMN IF NOT EXISTS precombine_chime_included boolean DEFAULT false;

-- 3. Update get_device_config to respect the chime-included flag
