-- Migration: Serve custom TTS text as stream URLs in get_device_config
-- Description: Generates free Google TTS stream URLs for custom text announcements,
-- allowing ESP32 to stream TTS audio seamlessly without firmware changes or memory overhead.

CREATE OR REPLACE FUNCTION public.urlencode(p_str text)
RETURNS text AS $$
DECLARE
    out_str text := '';
    c text;
...
BEGIN
    ...
    FOR i IN 1..length(p_str) LOOP
        ...
        ELSIF c = ' ' THEN
            out_str := out_str || '%20';
        ELSE
            out_str := out_str || '%' || upper(lpad(to_hex(ascii(c)), 2, '0'));
        END IF;
    END LOOP;
    RETURN out_str;
END;
