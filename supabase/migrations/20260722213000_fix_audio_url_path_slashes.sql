-- Migration: Fix audio_url path slashes in get_device_config
-- Description: Ensures get_device_config preserves forward slashes '/' in storage_path (e.g. folder/file.mp3)
-- while URL-encoding spaces (e.g. period%204.mp3), so Supabase Storage correctly resolves the bucket path and HTTP GET succeeds on ESP32.
        ...
        SELECT json_agg(sched) INTO v_schedule_data FROM (
            SELECT
                ...
                CASE
                    ...
                    -- For MP3 audio: return storage path with preserved slashes (e.g. folder/period%201.mp3)
                    ELSE
