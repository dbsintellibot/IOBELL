-- Migration: Fix Device Config Clean Audio URL
-- Description: Ensures get_device_config returns direct clean storage URLs for MP3 schedules
-- without wrapping in combine-audio, preventing double pre-announcement chimes.
        ...
        SELECT json_agg(sched) INTO v_schedule_data FROM (
            SELECT
                ...
                CASE
                    ...
                    -- For MP3 audio: return direct clean storage path (firmware plays native pre-announcement chime)
                    ELSE
