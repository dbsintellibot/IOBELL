-- Migration: URL-encode audio_url in get_device_config
-- Description: Ensures audio_url has spaces URL-encoded (e.g. period%201.mp3) so ESP32 HTTP client can fetch MP3 files from Supabase Storage.
        ...
        SELECT json_agg(sched) INTO v_schedule_data FROM (
            SELECT
                ...
                CASE
                    ...
                    -- For MP3 audio: return URL-encoded storage path (e.g. period%201.mp3)
                    ELSE
                        COALESCE(public.urlencode(MAX(af1.storage_path)), '')
                END as audio_url,
