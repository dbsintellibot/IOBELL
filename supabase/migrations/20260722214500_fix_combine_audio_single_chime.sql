-- Migration: Fix Scheduled Bell Audio Combining
-- Description: Wraps MP3 schedules in combine-audio Edge Function while turning off standalone pa_en in top-level config.
-- This ensures ESP32 receives a single continuous stream (Pre-announcement -> 3s delay -> Period MP3) without socket dropouts or duplicate chimes.
...
BEGIN
        ...
        SELECT json_agg(sched) INTO v_schedule_data FROM (
            SELECT
                ...
                CASE
                    ...
                    -- When pre-announcement is enabled and chime URL exists: use combine-audio Edge Function
                    WHEN v_pa_enabled AND v_pa_url IS NOT NULL AND v_pa_url != '' THEN
                        'https://hjlwzkwiweocnfztshmy.supabase.co/functions/v1/combine-audio?chime_url=' || public.urlencode(v_pa_url) || '&delay_sec=' || v_pa_delay || '&audio_url=' || public.urlencode(COALESCE(MAX(af1.storage_path), ''))
                    -- For MP3 audio without pre-announcement: return clean storage path (e.g. folder/period%201.mp3)
                    ELSE
    ...
    -- Set pa_en = false and pa_url = '' in top-level JSON so ESP32 firmware doesn't play double chimes.
    -- The pre-announcement chime + delay + period MP3 is handled seamlessly by combine-audio.
