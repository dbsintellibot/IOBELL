-- Migration: Combine Pre-Announcement Chime + Delay + Main Audio Stream
-- Description: Automatically wraps scheduled announcements with the school's configured pre-announcement chime and delay
-- using the combine-audio Edge Function, ensuring pre-announcements play before scheduled TTS and MP3 announcements.
    ...
    BEGIN
        ...
        SELECT json_agg(sched) INTO v_schedule_data FROM (
            SELECT
                ...
                CASE
                    WHEN t.play_type = 'tts' AND COALESCE(t.audio_file_id::text, '') = '' AND t.tts_message IS NOT NULL AND t.tts_message != '' THEN 'mp3'
                    ELSE t.play_type
                END as ty,
                COALESCE(MAX(t.tts_message), '') as tt,
                COALESCE(bool_or(t.include_weather), false) as iw,
                CASE
                    -- When pre-announcement is enabled and default sound URL is configured for the school
                    WHEN v_pa_enabled AND v_pa_url IS NOT NULL AND v_pa_url != '' THEN
                        'https://hjlwzkwiweocnfztshmy.supabase.co/functions/v1/combine-audio?chime_url=' || public.urlencode(v_pa_url) || '&delay_sec=' || v_pa_delay || '&audio_url=' || public.urlencode(
                            CASE
                                WHEN t.play_type = 'tts' AND COALESCE(t.audio_file_id::text, '') = '' AND t.tts_message IS NOT NULL AND t.tts_message != '' THEN
                                    'https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=en&q=' || public.urlencode(t.tts_message)
                                ELSE
                                    MAX(af1.storage_path)
                            END
                        )
                    -- When pre-announcement is disabled or no default sound URL is set
                    ELSE
                        CASE
                            WHEN t.play_type = 'tts' AND COALESCE(t.audio_file_id::text, '') = '' AND t.tts_message IS NOT NULL AND t.tts_message != '' THEN
                                'https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=en&q=' || public.urlencode(t.tts_message)
